...
public class Session5 extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		System.out.println("Session5.init() 실행");
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("Session5.doGet() 실행");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("Session5.doPost() 실행");
		doHandle(request, response);
	...
	private void doHandle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (session.isNew()) {	// 최초 요청 시 수행
			...
			if (userId != null) {	// 로그인 창에서 서블릿으로 요청했다면 ID가 null이 아니므로 세션에 ID를 바인딩 함
				...
				String url = response.encodeURL("session5");				// 변수 url에 encodeURL()을 이용해 응답 시 미리 jsessionId를 저장
				out.println("	<a href='" + url + "'>로그인 상태 확인</a>");	// 로그인 상태 확인 클릭 시 jsessionId를 서블릿으로 다시 전송

			} else {

				out.println("	<a href='/servlet/views/login/session5.html'>다시 로그인 하세요!! 전송된 id가 없음</a>");
			...
			if (userId != null && userId.length() != 0) {
				...
				out.println("	<a href='/servlet/views/login/session5.html'>다시 로그인 하세요!! 사용자 정보 없음</a>");
				session.invalidate();	// 생성된 세션 객체를 강제 삭제
	...
	public void destroy() {
		...
		System.out.println("Session5.destroy() 실행");
